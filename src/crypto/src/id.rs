...
impl TxId {
    ...
    pub fn try_from_base64(input: &str) -> std::result::Result<Self, NanoError> {
        Self::try_from_bytes(base64ct::Base64::decode_vec(input).unwrap().as_slice())
    }

    pub fn try_from_bytes(data: &[u8]) -> std::result::Result<Self, NanoError> {
        let arr: [u8; TX_ID_LENGTH] = data.try_into().map_err(|_| NanoError::InvalidAddress)?;
        Ok(Self { data: arr })
    }
}
...
impl BillId {
    pub fn new(block_id: u64, mutation_id: u16) -> Result<Self> {
        ...
        let data_array: [u8; BILL_ID_LENGTH] = data
            .try_into()
            .map_err(|_| NanoError::KeyCodecError("invalid array length".to_string()))?;
...
impl TryFrom<&[u8]> for BillId {
    ...
    fn try_from(data: &[u8]) -> Result<Self> {
        let data_array: [u8; BILL_ID_LENGTH] = data
            .try_into()
            .map_err(|_| NanoError::KeyCodecError("array length is invalid".to_string()))?;
        Ok(BillId { data: data_array })
    }
}
...
impl DocumentId {
    ...
    pub fn get_collection_id(&self) -> std::result::Result<CollectionId, NanoError> {
        CollectionId::try_from_bytes(
            self.data[TYPE_ID_LENGTH..TYPE_ID_LENGTH + OP_ENTRY_ID_LENGTH].as_ref(),
        )
    }

    pub fn get_document_entry_id(&self) -> std::result::Result<DocumentEntryId, NanoError> {
    ...
    pub fn try_from_bytes(data: &[u8]) -> std::result::Result<Self, NanoError> {
        let buf: [u8; DOCUMENT_ID_LENGTH] = data
            .try_into()
            .map_err(|_| NanoError::InvalidDocumentIdBytes)?;
        Ok(Self { data: buf })
    }
    ...
    pub fn try_from_base64(input: &str) -> std::result::Result<Self, NanoError> {
        let b64_vec =
            base64ct::Base64::decode_vec(input).map_err(|_| NanoError::InvalidDocumentIdBytes)?;
...
impl FieldKey {
    ...
    fn add_encode_field<T>(&mut self, v: &T) -> std::result::Result<(), NanoError>
    where
        T: Serialize + ?Sized,
    {
        let buf =
            storekey::serialize(v).map_err(|e| NanoError::DocumentDecodeError(format!("{e}")))?;
        self.data.extend_from_slice(buf.as_ref());
        Ok(())
    }
    fn add_field(&mut self, field: &Option<Bson>) -> std::result::Result<(), NanoError> {
        match field {
            None => {
            ...
            Some(v) => {
                return Err(NanoError::DocumentDecodeError(format!(
    ...
    pub fn create(fields: &Vec<Option<Bson>>) -> std::result::Result<Self, NanoError> {
    ...
    pub fn create_single_key(field: Option<Bson>) -> std::result::Result<Self, NanoError> {
        Self::create(&vec![field])
    }
    fn read_next_field(
        cursor: &mut Cursor<Vec<u8>>,
    ) -> std::result::Result<Option<Bson>, NanoError> {
        let field_id = cursor
            .read_u8()
            .map_err(|e| NanoError::DocumentDecodeError(format!("{e}")))?;

        let field_type = FieldTypeId::from_u8(field_id)
            .ok_or_else(|| NanoError::DocumentDecodeError(format!("field type is not supported")))?;
        match field_type {
            ...
            _ => Err(NanoError::DocumentDecodeError(
                "field type is not supported".to_string(),
            )),
        }
    }
    pub fn extract_fields(&self) -> std::result::Result<Vec<Option<Bson>>, NanoError> {
        if self.data.len() == 0 {
            return Ok(Vec::new());
        }
...
impl IndexId {
    ...
    pub fn get_document_id(&self) -> std::result::Result<DocumentId, NanoError> {
    ...
    pub fn get_key(&self) -> std::result::Result<FieldKey, NanoError> {
...
pub const DBID_LENGTH: usize = NANO_ADDRESS_LENGTH;
#[derive(Eq, Default, PartialEq, Ord, PartialOrd, Copy, Clone)]
pub struct DbId {
    addr: NanoAddress,
}
impl DbId {
    ...
    #[inline]
    pub fn min_id() -> DbId {
        DbId::from(&[std::u8::MIN; NANO_ADDRESS_LENGTH])
    }

    #[inline]
    pub fn max_id() -> DbId {
        DbId::from(&[std::u8::MAX; NANO_ADDRESS_LENGTH])
    }
...
impl From<&[u8; NANO_ADDRESS_LENGTH]> for DbId {
    fn from(data: &[u8; NANO_ADDRESS_LENGTH]) -> Self {
        Self {
            addr: NanoAddress::from(data),
        }
    }
}
...
impl TryFrom<&str> for DbId {
    ...
    fn try_from(addr: &str) -> std::result::Result<Self, NanoError> {
        Ok(Self {
            addr: NanoAddress::try_from(addr)?,
        })
    }
}
...
impl From<NanoAddress> for DbId {
    fn from(addr: NanoAddress) -> Self {
        Self { addr }
    }
}

impl TryFrom<(&NanoAddress, u64)> for DbId {
    type Error = NanoError;
    fn try_from(input: (&NanoAddress, u64)) -> std::result::Result<Self, NanoError> {
...
mod tests {
    ...
    fn test_ts_db_id_smoke() {
        let sender = NanoAddress::try_from("0xed17b3f435c03ff69c2cdc6d394932e68375f20f").unwrap();
        let nonce: u64 = 10;
    ...
    fn bson_into_comparison_bytes(field: &Bson) -> std::result::Result<FieldKey, NanoError> {
